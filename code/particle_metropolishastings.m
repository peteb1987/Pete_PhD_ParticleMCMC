function [ mc ] = particle_metropolishastings( display, algo, known, observ )
%PARTICLE_METROPOLISHASTINGS Run particle MMH algorithm to estimate 
%parameters and states from observations.
...
param_prior = known.paramprior(algo, known, param);
...
% Sample a trajectory
[traje, bess] = sample_trajectory(algo, model, pf, observ);
...
for rr = 2:algo.R
    ...
    [ppsl_param, ppsl_forw_prob, ppsl_back_prob] = model.paramproposal(algo, known, param, traje, observ);
    ppsl_param_prior = model.paramprior(algo, known, ppsl_param);
    
    % Merge known and unknown parameters
    ppsl_model = catstruct(known, ppsl_param);
    
    % Run conditional particle filter
    pf = pf_standard(algo, ppsl_model, observ);
