...
\name{Pete Bunch and Fredrik Lindsten}
...
\end{IEEEeqnarray}
%
Note that each of these operations is a collapsed Gibbs move, since not all the remaining variables are conditioned upon. Specifically, we have excluded the future states and ancestors other than those in the reference trajectory. The justification for this is that conceptually we are actually sampling these missing variables from the correct joint conditional distribution. However, since no future operation will depend on them, we need not actually bother generating them. See \cite{Dyk2008}.
...
In standard backward simulation, the conditional for each ancestor index is a categorical distribution \eqref{eq:bs-distn}, which can be sampled directly by evaluating the weight associated with each possible value, or by rejection sampling \cite{Douc2009,Taghavi2013}. It is also possible to use a Metropolis-Hastings kernel targeting this distribution \cite{Bunch2013,Bunch2014}.
...
\end{IEEEeqnarray}
%
This is the scheme suggested in \cite{Bunch2013,Bunch2014}.
